...
{% hint style="info" %}
Request interceptors are called just before sending a request.
{% endhint %}
...
In some cases you may run into a case where it's not possible to continue within an interceptor and want to break
out/cancel the request. This can be achieved by throwing an exception. This will not return a response and the request
...
class AuthInterceptor implements Interceptor {
  ...
  FutureOr<Response<BodyType>> intercept<BodyType>(Chain<BodyType> chain) async {
    ...
    if (response.statusCode == 401) {
      ...
      if (!isRefreshed) {
        // Throw an exception to stop the request.
...
It is not strictly necessary to throw an exception in order to break out of the
interceptor. Other approaches can also be used depending on how the project is
structured, such as calling an injected service or providing a callback that
...
To see logging in the console, the logging package also needs to be added to your project and configured.
